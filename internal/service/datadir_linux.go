...
func inspectDataDir(path string) (dataDirPlan, error) {
	...
	if linfo.uid != uid {
		closeFdSeam(fd)
		closeFdSeam(leafFd)
		return dataDirPlan{}, fmt.Errorf("data directory %q already exists and is owned by UID %d; the trestle service requires it to be owned by %s:%s with mode 0700. Move existing data under %s or re-home it; the installer will not adopt an existing directory", path, linfo.uid, ServiceUser, ServiceGroup, DefaultDataDir)
	}
	if e := parentSafeSeam(fd); e != nil {
		closeFdSeam(fd)
		closeFdSeam(leafFd)
