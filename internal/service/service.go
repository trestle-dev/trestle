...
func establishDataDir(path string) error {
	...
	if errors.Is(e, os.ErrNotExist) {
		...
		if e := chmodPath(clean, 0o700); e != nil {
			_ = os.Remove(clean)
...
// owned by the service account. It is a variable so tests can simulate
// ownership without real chown privileges.
...
func Install(exe, dataDir, listen string, envfile string) (retErr error) {
	...
	dataState, e := inspectDataDir(dataDir)
	if e != nil {
		return e
	}
	...
	// Genuine no-op decision is prerequisite-aware: an identical managed unit,
	// identical binary and already-preserved enabled+active state are a no-op
	// only when the data directory prerequisite is safely present. A safely
	// missing leaf is repaired (not claimed correct); an unsafe leaf was
	// already refused above.
	preserved := hadUnit && !unitChanged && !binaryChanged && priorEnabled == "enabled" && priorActive == "active"
	...
	if e := establishDataDir(dataDir); e != nil {
		return e
	}
	if repairDataOnly {
		return nil
