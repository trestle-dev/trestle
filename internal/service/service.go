...
// and binary.
func Install(exe, dataDir, listen string, envfile string) error {
	...
	restore := func() string {
		...
		if hadUnit {
			if priorEnabled == "enabled" {
				if e := systemctlSuccess("enable", "trestle.service"); e != nil {
			...
			if priorActive == "active" {
				if e := systemctlSuccess("start", "trestle.service"); e != nil {
