...
func TestInstallFreshAndChanged(t *testing.T) {
	t.Run("fresh install publishes and starts", func(t *testing.T) {
		...
		for _, want := range []string{"daemon-reload", "enable trestle.service", "restart trestle.service"} {
	...
	t.Run("changed configuration restarts the service", func(t *testing.T) {
		...
		fs := newFakeSystemd(m.unitPath)
		m.run = fs.runner()
		if err := m.install("127.0.0.1:8090", filepath.Join(t.TempDir(), "data1"), "", os.Stderr); err != nil {
			t.Fatal(err)
		}
		fs.setState("enabled", "active")
		fs.calls = nil
		if err := m.install("127.0.0.1:8095", filepath.Join(t.TempDir(), "data2"), "", os.Stderr); err != nil {
			t.Fatalf("changed reinstall: %v", err)
		}
		if !fs.callsContain("restart trestle.service") {
	...
	t.Run("unchanged unit on inactive service starts it", func(t *testing.T) {
		...
		if !fs.callsContain("start trestle.service") {
	...
	t.Run("unchanged unit on disabled service enables then starts", func(t *testing.T) {
		...
		if !fs.callsContain("enable trestle.service") || !fs.callsContain("start trestle.service") {
...
func TestInstallRollbackMatrix(t *testing.T) {
	...
	for _, p := range pairs {
		t.Run("enabled="+p.enabled+"/active="+p.active, func(t *testing.T) {
			...
			fs.calls = nil
			if !p.restorable {
				err := m.install("127.0.0.1:8096", filepath.Join(t.TempDir(), "data2"), "", os.Stderr)
				if err == nil {
			...
			fs.failVerb = "restart"
			err := m.install("127.0.0.1:8097", filepath.Join(t.TempDir(), "data3"), "", os.Stderr)
			if err == nil {
...
func TestInstallRestoresEnablementLayers(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run("prior="+tc.prior, func(t *testing.T) {
			...
			fs.failVerb = "restart"
			if err := m.install("127.0.0.1:8097", filepath.Join(t.TempDir(), "data3"), "", os.Stderr); err == nil {
...
func TestInstallFailureRestoresPriorState(t *testing.T) {
	...
	t.Run("fresh install", func(t *testing.T) {
		for _, st := range steps {
			t.Run(st.verb, func(t *testing.T) {
				...
				m.run = fs.runner()
				fs.failVerb = st.verb
				err := m.install("127.0.0.1:8090", filepath.Join(t.TempDir(), "data"), "", os.Stderr)
				if err == nil {
	...
	t.Run("reinstall restores prior unit and lifecycle", func(t *testing.T) {
		for _, st := range steps {
			t.Run(st.verb, func(t *testing.T) {
				...
				fs.failVerb = st.verb
				err := m.install("127.0.0.1:8097", filepath.Join(t.TempDir(), "data2"), "", os.Stderr)
				if err == nil {
	...
	t.Run("reinstall failure at restart restores enabled active prior", func(t *testing.T) {
		...
		fs.setState("enabled", "active")
		fs.failVerb = "restart"
		if err := m.install("127.0.0.1:8098", filepath.Join(t.TempDir(), "data2"), "", os.Stderr); err == nil {
