...
// trestleManagedPrefix introduces the versioned integrity header. The header
...
// trestleHealthPath is the public, read-only liveness endpoint the service
...
func writeManagedUnit(path, content string) error {
	...
	tmp, err := os.CreateTemp(dir, ".trestle-unit-*")
	if err != nil {
		return err
	}
	tmpName := tmp.Name()
...
func resolveExecutable(exe string) (string, error) {
	...
	if strings.HasPrefix(abs, os.TempDir()) {
		return "", fmt.Errorf("executable path %q is transient; install trestle somewhere stable first", abs)
	}
...
func (m *serviceManager) install(listen, dataDir, envfile string, out io.Writer) error {
	...
	if changed {
		...
		// Restart (not start) so a changed listener, data directory or
		// environment file takes effect on an already-running service.
...
func writeFileAtomic(path string, data []byte, mode os.FileMode) error {
	...
	tmp, err := os.CreateTemp(dir, ".trestle-restore-*")
	if err != nil {
		return err
	}
	name := tmp.Name()
...
func (m *serviceManager) uninstall(out io.Writer) error {
	...
	fmt.Fprintf(out, "Removed %s. Trestle data and configuration were preserved.\n", m.unitName)
	return nil
}
...
func runService(args []string, version string) int {
	...
	fs := flag.NewFlagSet("trestle service "+cmd, flag.ContinueOnError)
	...
	if *system {
		fmt.Fprintln(os.Stderr, "trestle: system-wide service mode is not yet supported; use user mode (default) or the foreground command")
		return 2
	}
	if runtime.GOOS != "linux" {
		fmt.Fprintln(os.Stderr, "trestle: service installation requires Linux systemd")
		return 2
	}
	...
	*listen, *dataDir, *envFile = resolveInstallValues(meta, visited, *listen, *dataDir, *envFile)
	...
	case "uninstall":
		...
		return 0
	default:
		fmt.Fprintf(os.Stderr, "trestle: unknown service command %q\n\nUsage: trestle service <install|start|stop|restart|status|logs|uninstall> [flags]\n", cmd)
		return 2
	}
}
